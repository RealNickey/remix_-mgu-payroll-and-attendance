...
export function generateSummaryReport(
  ...
  doc.text(`Disbursement Summary — ${monthName} ${year}`, pageWidth / 2, 34, { align: 'center' });
  ...
  doc.text(`Report Date: ${new Date().toLocaleDateString('en-GB')}`, pageWidth - margin, 44, { align: 'right' });
  ...
  autoTable(doc, {
    startY: 50,
    head: [['Sl. No.', 'Employee Name', 'Category', 'Bank Account Number', 'Total Payable']],
    body: tableData,
    theme: 'striped',
    headStyles: {
...
export function generateAttendanceReport(
  ...
  doc.text(`General Attendance Report — ${monthName} ${year}`, pageWidth / 2, 34, { align: 'center' });

  // Report Date
  doc.setFont('Helvetica', 'normal');
  doc.setFontSize(10);
  doc.setTextColor(100);
  doc.text(`Generated on: ${new Date().toLocaleDateString('en-GB')}`, margin, 44);
  ...
  autoTable(doc, {
    startY: 50,
    head: [['Sl. No.', 'Employee Name', 'Category', 'Total Days Attended', 'OT Days']],
    body: tableData,
    theme: 'striped',
    headStyles: {
      fillColor: [15, 23, 42],
...
export function generateEmployeeReceipt(
  ...
  doc.setFontSize(12);
  doc.setTextColor(15, 23, 42);
  doc.text(`Attendance sheet for the month of ${monthName} ${year}`, pageWidth / 2, 26, { align: 'center' });
