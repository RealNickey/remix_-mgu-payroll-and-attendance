...
import type { Employee, Contract, AttendanceData, WageSettings, JobCategory, AttendanceRecord } from './types';
import { computeEndDate, getBillingCycleDates, formatDateKey, isDateCoveredByContract, doIntervalsOverlap } from './payrollUtils';
...
interface MguDbContextType {
  ...
  addEmployee: (name: string, category: JobCategory, bankAccount: string) => void;
  deleteEmployee: (id: string) => void;
  addContract: (employeeId: string, startDate: string, goNumber: string, goDate: string) => void;
  voidContract: (id: string) => void;
  updateAttendance: (employeeId: string, dateStr: string, record: Partial<AttendanceRecord>) => void;
...
export const MguDbProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Actions
  const addEmployee = (name: string, category: JobCategory, bankAccount: string) => {
  ...
  const addContract = (employeeId: string, startDate: string, goNumber: string, goDate: string) => {
  ...
  const updateAttendance = (employeeId: string, dateStr: string, recordUpdate: Partial<AttendanceRecord>) => {
    const employeeAttendance = attendance[employeeId] || {};
    const currentRecord = employeeAttendance[dateStr] || { fn: false, an: false, ot: false, holiday: false };
  ...
  const batchMarkWeekdays = (employeeId: string, dates: Date[]) => {
    ...
    dates.forEach(date => {
      ...
      if (isWeekday && isDateCoveredByContract(contracts, employeeId, dateStr)) {
        const currentRecord = employeeAttendance[dateStr] || { fn: false, an: false, ot: false, holiday: false };
  ...
  const calculatePayroll = (year: number, month: number): PayrollRow[] => {
    ...
    employees.forEach(emp => {
      ...
      const relevantContract = empContracts.length > 0 ? empContracts[empContracts.length - 1] : undefined;
