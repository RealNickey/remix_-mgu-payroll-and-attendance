...
export function computeEndDate(startDateStr: string): string {
  ...
  const end = new Date(start.getFullYear(), start.getMonth(), start.getDate() + 89);
...
export function getContractStatus(startDateStr: string, endDateStr: string): 'Upcoming' | 'Active' | 'Expired' {
...
export function isDateCoveredByContract(contracts: Contract[], employeeId: string, dateStr: string): boolean {
  return contracts.some(c => c.employeeId === employeeId && c.startDate <= dateStr && dateStr <= c.endDate);
}
